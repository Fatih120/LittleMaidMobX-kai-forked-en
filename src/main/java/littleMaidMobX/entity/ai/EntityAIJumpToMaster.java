...
public class EntityAIJumpToMaster extends EntityAIBase implements IEntityAI {
	...
	public EntityAIJumpToMaster(EntityLittleMaid pEntityLittleMaid) {
		super();
...
//				theMaid.func_110171_b(
			...
			/**/if (theMaid.getHomePosition().getDistanceSquared(
					MathHelper.floor_double(theMaid.posX),
					MathHelper.floor_double(theMaid.posY),
					MathHelper.floor_double(theMaid.posZ)) > EntityModeBase.limitDistance_Freedom) {
				jumpTarget = false;
			...
			if (theMaid.getAttackTarget() == null) {
				if (theMaid.mstatMasterDistanceSq < EntityModeBase.limitDistance_Follow) {
					return false;
				}
			} else {
				// ターゲティング中は距離が伸びる
