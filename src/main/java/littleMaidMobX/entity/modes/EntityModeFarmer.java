...
public class EntityModeFarmer extends EntityModeBase {

	public static final int mmode_Farmer = 0x0024;

	private int clearCount = 0;

	public EntityModeFarmer(EntityLittleMaid pEntity) {
		super(pEntity);
	}
	...
	public boolean changeMode(EntityPlayer pentityplayer) {
		if (!LittleMaidConfig.enableFarmerMode) return false;
		ItemStack litemstack = owner.maidInventory.getStackInSlot(0);
		if (litemstack != null) {
			if ((litemstack.getItem() instanceof ItemHoe) || (TriggerSelect.checkItem(owner.getMaidMaster(), "Hoe", litemstack)))
			{
	...
	public boolean checkItemStack(ItemStack pItemStack) {
		...
		return (pItemStack.getItem() instanceof ItemSeeds || TriggerSelect.checkItem(owner.getMaidMaster(), "Seeds", pItemStack));
	}
	...
	public boolean checkBlock(int pMode, int px, int py, int pz) {
		...
		if(isFarmedLand(px,py,pz)){
			...
			int p = EntityModeBase.Water_Radius * 3;
...
//		if(owner.worldObj.isRemote) return false;
		...
		if (curStack != null && curStack.getItem() != null &&
				curStack.getItem() instanceof ItemHoe && isUnfarmedLand(px,py,pz) &&
