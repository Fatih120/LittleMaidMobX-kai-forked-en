...
public class EntityModeMiner extends EntityModeBase {
	int targetX;
	int targetY;
	int targetZ;
	...
	public EntityModeMiner(EntityLittleMaid pEntity)
	{
		super(pEntity);
	}
	...
	public boolean checkBlock(int pMode, int px, int py, int pz)
	{
		...
		if (owner.isFreedom() && owner.getHomePosition().getDistanceSquared(px, py, pz) > EntityModeBase.limitDistance_Freedom)
		{
			return false;
		}
		if (!owner.isFreedom() && owner.getMaidMasterEntity()!=null && owner.getMaidMasterEntity().getDistanceSq(px, py, pz) > EntityModeBase.limitDistance_Follow)
		{
			return false;
		}
		if (shouldBlockBeMined(px, py, pz) && canBlockBeSeen(px, py, pz, true, true, false) && isInvNotFull() && !owner.isMaidWait())
		{
			...
			{
				owner.playSound(EnumSound.findTarget_N, false);
				return true;
			}
		}
		return false;
	...
	{
		if (owner.isFreedom() && owner.getHomePosition().getDistanceSquared(targetX, targetY, targetZ) > EntityModeBase.limitDistance_Freedom)
		{
			return false;
		}
		if (!owner.isFreedom() && owner.getMaidMasterEntity()!=null && owner.getMaidMasterEntity().getDistanceSq(targetX, targetY, targetZ) > EntityModeBase.limitDistance_Follow)
		{
			return false;
		}
		World worldObj = owner.worldObj;
		...
		if (owner.getDistanceSq(targetX, targetY, targetZ) < 16D)
		{
			...
			owner.setSwing(10, EnumSound.findTarget_B);
